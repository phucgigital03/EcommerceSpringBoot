    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(auth ->
                                auth.requestMatchers("/api/auth/enable-2fa").authenticated()
                                        .requestMatchers("/api/auth/disable-2fa").authenticated()
                                        .requestMatchers("/api/auth/verify-2fa").authenticated()
                                        .requestMatchers("/api/auth/user/2fa-status").authenticated()
                                        // 4 requests above for 2FA authentication
